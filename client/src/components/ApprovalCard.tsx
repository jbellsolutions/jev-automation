...
/** The brain asked before doing something; typing "yes"/"no" answers it too. */
export function ApprovalCard({ pending, onChoose }: { pending: NonNullable<Approval>; onChoose: (choice: ApprovalChoice) => void }) {
  ...
  return (
    <div id="approval" className="card approval">
      <div className="card-title">Brain needs your OK</div>
      <div className="card-body">{pending.question}</div>
      <div className="muted">Type "yes" or "no", or pick below.</div>
      <div className="row">
        {allow.map((c, i) => (
