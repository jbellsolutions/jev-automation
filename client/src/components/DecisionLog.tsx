...
import { BrainCard } from "./BrainCard.tsx";
    ...
    <li className="entry">
      ...
      {d && (
        <div className="meta">
          ...
          {d.route && d.route !== "browser_now" && <span className={`tag route ${d.route}`}>{d.route === "hermes" ? "→ Brain" : d.route}{d.routeConfidence ? ` ${Math.round(d.routeConfidence * 100)}%` : ""}</span>}
      ...
      {entry.brain && <BrainCard brain={entry.brain} />}
      {d && entry.result && (
