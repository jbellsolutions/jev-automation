...
export function selectSttProvider(env: NodeJS.ProcessEnv = process.env, opts: { appleBin?: string } = {}): SttProvider | null {
  ...
  const appleBin = env.JEV_SPEECH_BIN || opts.appleBin;
  const apple = appleSpeechAvailable(appleBin) ? new AppleSpeechProvider({ bin: appleBin }) : null;
